            ...
            public void getResult(String result) {
                try{
                    ...
                    for(int i = 0;i < array.length();i++){
                        ...
                        Integer tempId = Integer.parseInt(object.getString("id"));
                        myFollowersIDs.add(tempId);
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
            ...
            public void getResult(String result) {
                try {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        tempUser.setName(jsonObject.getString("username"));
                        tempUser.setId(Integer.parseInt(jsonObject.getString("userid")));

                        Log.e("test", tempUser.getId() + "------" + user.getId());
                        if(tempUser.getId().intValue() != user.getId().intValue())
                              users.add(tempUser);
                    }
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            if(myFollowersIDs.contains(users.get(position).getId())){
                ...
                button.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        HashMap<String, String> tempParameters = new HashMap<String, String>();
                        tempParameters.put("srcid", user.getId().toString());
                        tempParameters.put("dstid", users.get(position).getId().toString());
                        Connection conn = new PostConnection(tempParameters, new ConnectionListener() {
                            @Override
                            public void getResult(String result) {
                                try {
                                    JSONObject object = new JSONObject(result);
